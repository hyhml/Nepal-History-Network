...
`materials/local/` contains the scanned book and a local copy of the research data; it is excluded from Git. The tracked `examples/` directory contains structured facts and the source excerpts used by the public chart, as requested by the user. Do not add the local PDF, local duplicate dataset, `output/` HTML, or virtual environment to a commit. `docs/index.html` is the intended tracked public artifact.
